...
function updateFeeBadges(fees) {
    ...
    document.querySelectorAll('.btn-request').forEach(btn => {
        ...
        if (fees[title]) {
            ...
            if (cardInfo && title !== 'Vehicle Sticker Application') {
    ...
    if (tbody) {
        if (!dash.requests || dash.requests.length === 0) {
            ...
            tbody.innerHTML = dash.requests.map(r => {
                const canCancel = ['Pending', 'Submitted'].includes(r.status);
                ...
                return `
                    <tr>
                        ...
                        <td><span class="status-badge ${r.status === 'Cancelled' ? 'status-cancelled' : (r.status === 'Pending' ? 'status-pending' : 'status-completed')}">${escapeHtml(r.status)}</span></td>
                        <td>${actionBtn}</td>
                    </tr>
                `;
            }).join('');
...
async function executeRequestCancellation() {
    ...
    try {
        const response = await fetch('/api/requests/cancel', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ request_id: requestToCancelId })
        });
