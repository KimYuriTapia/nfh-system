...
def get_settings(keys=None):
    ...
    try:
        with conn.cursor() as cursor:
            if keys:
                format_placeholders = ','.join(['%s'] * len(keys))
                cursor.execute(f"SELECT setting_key, setting_value FROM hoa_settings WHERE setting_key IN ({format_placeholders})", tuple(keys))
            else:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""INSERT INTO hoa_settings (setting_key, setting_value, updated_by)
                              VALUES (%s,%s,%s)
                              ON CONFLICT (setting_key) DO UPDATE SET setting_value=%s, updated_by=%s""",
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT r.id, CONCAT(u.first_name,' ',u.last_name) AS homeowner, r.request_type,
                       ...
                       TO_CHAR(r.date_submitted,'YYYY-MM-DD HH24:MI') AS date_submitted,
                       TO_CHAR(r.last_updated,'YYYY-MM-DD HH24:MI') AS last_updated,
                       r.user_id
    ...
    try:
        ...
        with conn.cursor() as cursor:
            def count(sql, params=()):
                ...
                res = cursor.fetchone()
                return int(res['total'] or 0) if res else 0
            ...
            if role == 'Admin':
                ...
                row = cursor.fetchone()
                outstanding = float(row['total'] or 0) if row else 0.0
                metrics = {
...
def create_officer():
    ...
    try:
        ...
        flash(f'Error creating officer or username/email already exists.', 'error')
    return redirect(url_for('admin'))
...
def report_audit_trail_pdf():
    ...
    if username:
        query += " AND username ILIKE %s"; params.append(f"%{username}%")
    if role:
        query += " AND role = %s"; params.append(role)
    if action:
        query += " AND action ILIKE %s"; params.append(f"%{action}%")
